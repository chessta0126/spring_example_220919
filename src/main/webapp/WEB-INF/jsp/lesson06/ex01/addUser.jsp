...
<body>
		...
		<!-- <form method="post" action="/lesson06/ex01/add_user"> -->
			...
			<div class="form-group">
				<label for="yyyymmdd">생년월일</label> <input type="text" id="yyyymmdd"
					name="yyyymmdd" class="form-control col-3" placeholder="예)19980126">
			</div>

			<div class="form-group">
				<label for="email">email</label> <input type="text" id="email"
					name="email" class="form-control col-3" placeholder="email을 입력하세요">
			</div>
	...
	<script>
		$(document).ready(function(){
			/* // 1) J-Query의 submit 기능 이용하기
			$('form').on('submit',function(e){
				// alert("submit");
				// e.preventDefault(); // submit 되는 것 막는다
				
				// validation
				let name = $('#name').val().trim();
				if(name.length == ''){
					alert("이름을 입력하세요");
					return false; // submit되지 않는다.
				}
